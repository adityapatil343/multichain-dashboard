...
export default function App() {
  ...
  const [rows, setRows] = useState<Record<string, Row>>(() =>
    Object.fromEntries(CHAINS.map(c => [c.key, { balance: '-', usd: null, loading: false }])) as any
  )
  ...
  const handleLookup = async () => {
    ...
    setRows(prev => {
      const draft = { ...prev }
      CHAINS.forEach(
        c => (draft[c.key] = { ...draft[c.key], loading: true, error: undefined, explorerUrl: c.explorer(checksum) })
      )
      return draft
    })
        ...
        try {
          ...
          if (tokenAddrs.length && c.multicall3) {
            ...
            const balances: bigint[] = (returnData as Hex[]).map((data: Hex) => {
              try {
                const [bal] = (client.decodeFunctionResult as any)({
                  abi: erc20Abi,
          ...
          setRows(prev => ({
            ...prev,
            [c.key]: { ...prev[c.key], loading: false, balance: pretty(native), usd: usdVal, tokens },
          }))
        } catch (e: any) {
          setRows(prev => ({
            ...prev,
            [c.key]: { ...prev[c.key], loading: false, error: e?.message || 'Error' },
          }))
        }
      })
    )
    ...
    <div className="min-h-screen w-full bg-gradient-to-b from-slate-50 to-white p-6">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-center">
          ...
          />
          <button className="btn" onClick={handleLookup} disabled={!valid}>
        ...
        <div className="mt-4 flex gap-2 items-center">
          <input
            className="input"
            placeholder="Optional: URL to tokenlist JSON (by chain key)"
            value={tokenListUrl}
            onChange={e => setTokenListUrl(e.target.value)}
          />
          <button className="btn" onClick={fetchTokensFromUrl}>
            Load Tokenlist
          </button>
          {/* Use BASE_URL so the link works on GitHub Pages */}
          <a
            className="btn"
            href={`${import.meta.env.BASE_URL}example-tokenlist.json`}
            target="_blank"
            rel="noreferrer"
          >
            Example JSON
                ...
                <div className="flex items-center justify-between">
                  ...
                  {address && r.explorerUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-sm inline-flex items-center gap-1 hover:underline text-slate-600"
                    >
                  ...
                  {r.loading ? (
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="text-sm text-slate-500">
                        {r.usd != null ? `≈ $${pretty(r.usd, 2)}` : 'USD price unavailable'}
                      </div>
                    </div>
                  )}
                </div>
